...
public class FilterIndex implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        session.setAttribute("SERVSTAT", null);        
        session.setAttribute("STRUCTSAT", null);

        //Personnel P = Faces.getC
        if ("/index.xhtml".equals(req.getServletPath())) {
            if (pers == null) {
                System.out.println("nullUSER");
                res.sendRedirect("login.xhtml");
            } else {
                if (pers.isMedecin() || pers.isAdminService() || pers.isSecretaire()) {
                    res.sendRedirect("indexService.xhtml");
                } else if (pers.isAdminStructure()) {
                    res.sendRedirect("indexStructure.xhtml");
                }
